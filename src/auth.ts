...
export default class Auth {
    ...
    async generateToken() :Promise<[string,string]>{
        let used = true
        let relayId = newReadableId()
        while(used){
            const existing = await Storage.getItem(relayId)
            if(!existing){
                used = false
            } else {
        ...
        await Storage.setItem(relayId,true)
...
const newReadableId = () => {
    const index1 = crypto.randomInt(wordList.length)
    const index2 = crypto.randomInt(wordList.length)
    const index3 = crypto.randomInt(wordList.length)
